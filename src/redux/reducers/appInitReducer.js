...
const toIncludedCodes = (valueSets) => {
    ...
    if (valueSets) {
        return valueSets.filter( vs => vs.compose != undefined)
...
const valueSetsReducer = (state = valueSetsLoaderInitialState, action) => {
    switch (action.type) {
        ...
        case APP_INIT_ACTION_TYPES.FETCH_VALUESETS_SUCCESS:
            ...
            var purposesVSets = action.payload.filter(vs => (vs.resourceType === RESOURCE_TYPE_VALUESET && vs.id === HI_PUSPOSE_VALUESET));
            var purposeCodes = toIncludedCodes(purposesVSets); 
            if (purposeCodes) {
                newState.purposesOfUse = purposeCodes;
            }
            var hiTypesVSets = action.payload.filter(vs => (vs.resourceType === RESOURCE_TYPE_VALUESET && vs.id === HITYPE_TYPE_VALUESET));
